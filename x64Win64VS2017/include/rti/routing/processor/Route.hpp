...
public:
    ...
    /**
     *@brief Checks whether the Input at the specified index is enabled
     *
     */
    bool input_enabled(int32_t index) const
    {
        return RTI_RoutingServiceRoute_is_input_enabled(native_route_, index)
                ? true
                : false;
    }
    ...
    {
        return RTI_RoutingServiceRoute_is_output_enabled(native_route_, index)
                ? true
                : false;
    }
    ...
    /**
     *
     * @brief Changes the event period of this Route.
     *
     * This operation can be called many times within the same periodic event,
     * in which only the last call will have effect.
     *
     * The operation will enable the periodic event in this route if this
     * operation is called for the first time in a route with no period set
    ...
    {
        struct DDS_Duration_t native_period;
        rti::core::native_conversions::to_native(native_period, period);
    ...
    dds::core::Duration period() const
    {
        ...
        return rti::core::native_conversions::from_native(native_period);
    }

    /**
     *
     * @brief Returns the fully qualified of this Route, derived from the configuration.
     *
     */
...
private:
    ...
    Route(RTI_RoutingServiceRoute *native_route,
        ...
        full_name_(RTI_RoutingServiceRoute_get_full_name(native_route))
    {
    }
    ...
    Input* get_input_at_index(int32_t index)
    {
        RTI_RoutingServiceInput *native_input =
                RTI_RoutingServiceRoute_get_input_at(native_route_, index);
        if (native_input == NULL) {
            throw dds::core::InvalidArgumentError(
                    RTI_RoutingServiceEnvironment_get_error_message(native_env_));
        }

        return static_cast<Input*>(RTI_RoutingServiceInput_get_user_data(native_input));
    }

    Output* get_output_at_index(int32_t index)
    {
        RTI_RoutingServiceOutput *native_output =
                RTI_RoutingServiceRoute_get_output_at(native_route_, index);
        if (native_output == NULL) {
            throw dds::core::InvalidArgumentError(
                    RTI_RoutingServiceEnvironment_get_error_message(native_env_));
        }

        return static_cast<Output*>(RTI_RoutingServiceOutput_get_user_data(native_output));
    }
