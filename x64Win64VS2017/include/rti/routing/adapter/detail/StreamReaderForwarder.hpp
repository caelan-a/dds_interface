        ...
        RTI_RoutingServiceEnvironment *environment)
    {
        ...
        *array_length = 0;
        try {
            ...
            /*
             * We reserve a hidden slot in the sample_seq to keep the original
             * loaned returned from the native StreamReader. This allows us
             * to quickly access it on return_loan(), instead of having to search
             * in a list.
             */
            holder->sample_seq_.reserve(holder->sample_seq_.size() + 1);
            *(&holder->sample_seq_[0] + holder->sample_seq_.size()) = holder;
        } catch (const std::exception& ex) {
            RTI_RoutingServiceEnvironment_set_error(
                    environment,
                    "%s",
        ...
        // direct mapping
        *array_length = static_cast<int>(holder->sample_seq_.size());
