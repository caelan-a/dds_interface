...
#ifndef RTI_ROUTING_DETAIL_FORWARDER_UTILS_UTILS_HPP_
#define RTI_ROUTING_DETAIL_FORWARDER_UTILS_UTILS_HPP_
...
#define RTI_ROUTING_SAMPLE_VECTOR_COPY_FROM_NATIVE_W_OFFSET( \
        VECTOR, \
        NATIVE_ARRAY, \
        OFFSET, \
        ARRAY_LENGTH) \
    (VECTOR).resize((ARRAY_LENGTH) + (OFFSET)); \
    memcpy(&((VECTOR)[OFFSET]), \
            (NATIVE_ARRAY), \
            sizeof (void*) * (ARRAY_LENGTH));
...
#endif // RTI_ROUTING_DETAIL_FORWARDER_UTILS_UTILS_HPP_
