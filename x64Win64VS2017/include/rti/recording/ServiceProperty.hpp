...
struct ApplicationRoleKind_def {
    ...
    enum type {
        ...
        /*i
         * @brief Indexer mode. To be used internally by our command-line tool.
         */
        INDEX_APPLICATION,
        /*i
         */
...
/*i
 * @brief Indexer mode enumeration, used by the index application (offline
 * indexing)
 */
struct IndexerModeKind_def {
...
typedef dds::core::safe_enum<IndexerModeKind_def> IndexerModeKind;

typedef rti::routing::PropertySet PropertySet;
...
public:
    ...
    ServiceProperty() :
        ...
        output_cfg_file_(""),
        debug_mode_(false),
        xml_converter_role_(XmlConverterRoleKind::RECORD_CONVERT),
        indexer_mode_(IndexerModeKind::INSTANCE_INDEXING)
    ...
    /**
     * @brief Set this to true to enable debug mode
     * or false to disable it.
     *
     * @default false
     */
     ...
     * @brief Internal use only
     */
    XmlConverterRoleKind xml_converter_role() const
    {
        return xml_converter_role_;
    }

    /*i
     * @brief Internal use only
     */
    ServiceProperty& xml_converter_role(
            const XmlConverterRoleKind & xml_converter_role)
    {
        xml_converter_role_ = xml_converter_role;
        return *this;
    }

    /*i
     * @brief The indexer application mode. Defined by the IndexerModeKind.
     * Internal use only
     */
    IndexerModeKind indexer_mode() const
    {
        return indexer_mode_;
    }

    /*i
     * @brief Set the indexer application mode. Defined by the IndexerModeKind.
     * Internal use only
     */
    ServiceProperty& indexer_mode(const IndexerModeKind& indexer_mode)
    {
        indexer_mode_ = indexer_mode;
        return *this;
    }

    /*i
     * @brief The indexer database directory.
     * Internal use only
     */
    const std::string& db_directory() const
    {
        return db_directory_;
    }

    /*i
     * @brief Set the indexer database directory.
     * Internal use only
     */
    ServiceProperty& db_directory(const std::string& db_directory)
    {
...
private:
    ...
    bool debug_mode_;
    XmlConverterRoleKind xml_converter_role_;
    IndexerModeKind indexer_mode_;
    std::string db_directory_;
    PropertySet user_env_;
};
