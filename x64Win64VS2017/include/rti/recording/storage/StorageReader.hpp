...
public:
    ...
    */
    ...
    virtual StorageStreamInfoReader * create_stream_info_reader(
            const rti::recording::PropertySet& properties) = 0;
     ...
     *           integer in text format) for this stream info reader to query
     *           data
     *        4) name='rti.recording_service.playback_settings.sample_order'
     *           value=the sample order (enum label in text format) for this
     *           stream info reader to query data.
     *           The possible values are: RECEPTION_TIMESTAMP, SOURCE_TIMESTAMP.
     */
