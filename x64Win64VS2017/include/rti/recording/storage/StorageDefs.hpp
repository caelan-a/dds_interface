...
public:
    ...
    {
        RTI_CHECK_PRECONDITION(storage_writer_parent != NULL);
    }
...
public:
    ...
    {
        RTI_CHECK_PRECONDITION(storage_reader_parent != NULL);
    }
...
inline std::string jump_timestamp_property_name()
{
    return RTI_RECORDING_SERVICE_JUMP_TIMESTAMP_PROPERTY_NAME;
}

inline std::string sample_order_property_name()
{
    return RTI_RECORDING_SERVICE_PLAYBACK_SAMPLE_ORDER_PROPERTY_NAME;
}
...
#define RecorderPrint_exception(...) \
    RTILogMessage_print( \
...
} /* namespace rti::recording::detail */
