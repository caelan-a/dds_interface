...
/*
 * @(#)pres_common_impl.h    generated by: makeheader    Fri Apr 23 00:10:13 2021
 *
...
struct PRESSampleSignature {
    ...
    /*
     * Important: these flags are currently only used internally, and not
...
#define PRES_TRUSTED_PARTICIPANT_INFO_INVALID { \
    ...
    PRESProductVersion_INITIALIZER \
}
...
#define PRESProductVersion_isDefault(__productVersion) \
  ((__productVersion)->major == MIG_RTPS_PRODUCT_VERSION_MAJOR_DEFAULT \
      && (__productVersion)->minor == MIG_RTPS_PRODUCT_VERSION_MINOR_DEFAULT \
      && (__productVersion)->release == MIG_RTPS_PRODUCT_VERSION_RELEASE_DEFAULT \
      && (__productVersion)->revision == MIG_RTPS_PRODUCT_VERSION_REVISION_DEFAULT)

/**
 * Checks if the serialization of the GUID in a directed write should be done
 * natively or non-natively.
 *
 * Product versions after Hercules (including it) use non-native serialization
 * for the GUID.
 *
 * Product versions before Hercules use native serialization for the GUID.
 *
 * If the vendor is not RTI, then the product version will be the default one.
 * As a consequence, directed write is not native.
 * This assumes that we have also checked already if the RTPS protocol is before
 * 2.4, in which case, we should ignore the directed write (if vendor not RTI).
 * See PRESRtps_ignoreDirectedWrite macro.
 */
#define PRESProductVersion_isDirectedWriteNative(__productVersion) \
  (!PRESProductVersion_isDefault((__productVersion)) \
      && ((__productVersion)->major == PRES_PRODUCT_VERSION_MAJOR_DIRECTED_WRITE_NATIVE \
          ? ((__productVersion)->minor == PRES_PRODUCT_VERSION_MINOR_DIRECTED_WRITE_NATIVE \
              ? ((__productVersion)->release == PRES_PRODUCT_VERSION_RELEASE_DIRECTED_WRITE_NATIVE \
                  ? ((__productVersion)->revision == PRES_PRODUCT_VERSION_REVISION_DIRECTED_WRITE_NATIVE \
                     ? RTI_FALSE \
                     : (__productVersion)->revision < PRES_PRODUCT_VERSION_REVISION_DIRECTED_WRITE_NATIVE) \
                  : (__productVersion)->release < PRES_PRODUCT_VERSION_RELEASE_DIRECTED_WRITE_NATIVE) \
              : (__productVersion)->minor < PRES_PRODUCT_VERSION_MINOR_DIRECTED_WRITE_NATIVE) \
...
/**
 * Ignore the PID_DIRECTED_WRITE inline QoS for vendors different than RTI
 * if the RTPS protocol version is before 2.4.
 */
#define PRESRtps_ignoreDirectedWrite(__protocolVersion, __vendorId) \
  ((__vendorId) != MIG_RTPS_VENDOR_ID_RTI \
      && (__protocolVersion) < MIG_RTPS_PROTOCOL_VERSION_2_4)
