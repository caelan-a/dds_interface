...
/*
 * @(#)pres_participant.h    generated by: makeheader    Fri Apr 23 00:10:19 2021
 *
...
/* Participant table names */
#define PRES_PARTICIPANT_TABLE_NAME_REMOTE_PARTICIPANT "remoteParticipant"
#define PRES_PARTICIPANT_TABLE_NAME_WRR "pres writer matching remote reader"
#define PRES_PARTICIPANT_TABLE_NAME_RRW "pres reader matching remote writer"


/* This property when enabled will allow the creation of a topic multiple times
 * assuming that the type name is different
 */
#define PRES_PARTICIPANT_ALLOW_DUPLICATE_TOPIC PROPERTY_NAME_DDS_PARTICIPANT_ALLOW_DUPLICATE_TOPIC
...
#define PRES_PARTICIPANT_USE_45D_COMPATIBLE_ALIGNMENT_ENFORCEMENT_PROPERTY_NAME \
    PROPERTY_NAME_DDS_PARTICIPANT_USE_45D_COMPATIBLE_ALIGNMENT_ENFORCEMENT


#define PRES_PARTICIPANT_INITIAL_AUTO_OBJECT_KEY_PROPERTY_NAME \
    PROPERTY_NAME_DDS_PARTICIPANT_INITIAL_AUTO_OBJECT_ID
...
#define PRES_PARTICIPANT_TRUST_PLUGINS_RTPS_PROTECTION_MAX_OVERHEAD_PROPERTY_NAME \
    PROPERTY_NAME_DDS_PARTICIPANT_TRUST_PLUGINS_RTPS_PROTECTION_MAX_OVERHEAD
#define PRES_PARTICIPANT_TRUST_PLUGINS_METADATA_PROTECTION_MAX_OVERHEAD_PROPERTY_NAME \
    PROPERTY_NAME_DDS_PARTICIPANT_TRUST_PLUGINS_METADATA_PROTECTION_MAX_OVERHEAD

#define PRES_PARTICIPANT_TRUST_PLUGINS_IS_NON_SECURE_PARTICIPANT_DISCOVERY_MUTABLE_PROPERTY_NAME \
    PROPERTY_NAME_DDS_PARTICIPANT_TRUST_PLUGINS_IS_NON_SECURE_PARTICIPANT_DISCOVERY_MUTABLE

#define PRES_PARTICIPANT_TRUST_PLUGINS_AUTHENTICATION_TIMEOUT_SEC_PROPERTY_NAME \
    PROPERTY_NAME_DDS_PARTICIPANT_TRUST_PLUGINS_AUTHENTICATION_TIMEOUT_SEC
#define PRES_PARTICIPANT_TRUST_PLUGINS_AUTHENTICATION_REQUEST_DELAY_SEC_PROPERTY_NAME \
    PROPERTY_NAME_DDS_PARTICIPANT_TRUST_PLUGINS_AUTHENTICATION_REQUEST_DELAY_SEC
/*
...
#define PRES_PARTICIPANT_TRUST_PLUGINS_COMPATIBILITY_MODE_VERSION_ID_PROPERTY_NAME \
    PROPERTY_NAME_DDS_PARTICIPANT_TRUST_PLUGINS_COMPATIBILITY_MODE_VERSION_ID
...
struct PRESParticipantSecurityAttributes {
    ...
    /*
     * When enabled, every participant discovery message sent over an unsecure
     * topic must include a valid sample signature (based on a preshared key).
     */
    RTIBool isNonSecureParticipantDiscoveryPresharedKeyProtectionEnforced;
...
{ \
    ...
    RTI_FALSE, /* isNonSecureParticipantDiscoveryPresharedKeyProtectionEnforced */ \
...
typedef enum {
    PRES_DIRECTED_WRITE_AUTO_SERIALIZATION,
    PRES_DIRECTED_WRITE_NEW_SERIALIZATION,
    PRES_DIRECTED_WRITE_OLD_SERIALIZATION
} PRESDirectedWriteSerializationKind;
...
struct PRESParticipantProperty {
    ...
    /*
     * Time-based logging authentication time threshold.
     * Default: INFINITE
     */
    RTINtpTime authenticationTimeThreshold;

    /*
     * CORE-10122: Property to control serialization of the GUID in directed
     * samples
     */
    PRESDirectedWriteSerializationKind directedWriteGuidSerializationKind;
};
...
*/
  #define PRES_PARTICIPANT_PROPERTY_DEFAULT { \
      ...
      {RTI_NTP_TIME_SEC_MAX, RTI_NTP_TIME_FRAC_AUTO}, /* authenticationTimeThreshold */ \
      PRES_DIRECTED_WRITE_AUTO_SERIALIZATION /* directedWriteGuidSerializationKind */ \
}
...
typedef RTIBool
(*PRESParticipant_TransformIncomingContentFunction)(
    struct PRESParticipant *participant,
    /* output */
...
extern PRESDllExport
RTIBool PRESParticipant_isNonSecureParticipantDiscoveryPresharedProtectionEnforced(
        struct PRESParticipant *me);
...
extern PRESDllExport
REDAUnicodeNormalizationKind PRESParticipant_getUnicodeNormalizationKind(
      const struct PRESParticipant *me);

extern PRESDllExport
REDACharacterEncodingKind PRESParticipant_getCharacterEncodingKind(
      const struct PRESParticipant *me);

extern PRESDllExport
RTIBool PRESParticipant_sqlFilterSupportsEscapedCharacters(
      const struct PRESParticipant *me);
...
extern PRESDllExport
const struct MIGRtpsGuid * PRESParticipant_getGuidRef(
        const struct PRESParticipant *me);

/*e \ingroup PRESParticipantModule

  Install local participant configuration listener on the participant.

  Setup local participant configuration listener for this participant.

  Only one local particpiant configuration listener can be attached to the
  participant. If a listener was already set, the operation will replace it
  with the new one.

  @pre Valid arguments.

  @param listener \b In. Local participant configuration listener to be set up.
  All methods on the listener are supposed to be defined if listener is not
  NULL. In the case when listener is NULL, it is equivalent to removing the
  local participant configuration listener.
...
extern PRESDllExport
RTIBool PRESParticipant_getRemoteParticipantProductVersion(
        const struct PRESParticipant *me,
        struct PRESProductVersion *remoteProductVersion,
        const MIGRtpsParticipantId *remoteParticipantId,
        struct REDAWorker *worker);

/*e \ingroup PRESParticipantModule

  Install local endpoint configuration listener on the participant for all
  local endpoints for the given service ID.
 
  Service need to be asserted to the participant before you can set up the
  local endpoint configuration listener for it.

  Only one local endpoint configuration listener can be attached to the
  service. If a listener was already set, the operation will replace it
  with the new one.

  @pre Valid arguments.

  @param serviceId \b In. Service ID to set the local endpoint configuration
  listener for.
  @pram listener \b In. Local endpoint configuration listener to be set up.
  All methods on the listener are supposed to be defined.

  @return RTI_TRUE if successful. RTI_FALSE otherwise, which can happen if
  there is no service asserted with the given service ID, or it fails to setup
  local endpoint configuration listener on the given service.
*/
extern PRESDllExport
...
extern PRESDllExport
RTIBool PRESParticipant_assertLiveliness(struct PRESParticipant *me);
...
extern PRESDllExport
PRESStatusKind PRESParticipant_getStatusChanges(struct PRESParticipant *me);
...
struct PRESSampleInfo {
    ...
    */
    ...
    /* This field is a pointer because it is an optional member in 
     * DDS_SampleInfo
     */
    struct PRESCoherentSetInfo *coherentSetInfo;
};

extern PRESDllExport
struct PRESSampleInfo *PRESSampleInfo_copy(
        struct PRESSampleInfo *dst,
        const struct PRESSampleInfo *src);

extern PRESDllExport 
void PRESSampleInfo_finalize(struct PRESSampleInfo *self);
...
extern	PRESDllExport
RTIBool PRESParticipant_printTable(
        struct PRESParticipant *me,
