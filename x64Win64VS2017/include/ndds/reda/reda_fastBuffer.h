...
/*
 * @(#)reda_fastBuffer.h    generated by: makeheader    Fri Apr 23 00:10:54 2021
 *
...
struct REDAThresholdBufferPool;
...
#define REDA_FAST_BUFFER_POOL_FLAGS_NONE (0x00)


#define REDA_FAST_BUFFER_POOL_FLAG_USE_IMPLICIT_TYPE_NAME (0x01)


typedef RTI_UINT32 REDAFastBufferPoolFlags;
...
extern REDADllExport
struct REDAFastBufferPool * REDAFastBufferPool_newWithParams(
    int bufferSize,
    RTIOsapiAlignment bufferAlignment,
...
typedef struct REDAThresholdBufferPoolProperty {
    /* 
     * The fixed size pool will be created with buffer with this size. Any
     * buffers requested from the pool with size greater than this threshold
     * value will be dynmically allocated.
     * 
     * threshold ==
     * 0             : all buffers are dynamically allocated
     * finite value  : buffers may be preallocated or dynamically allocated
     * UNLIMITED     : all buffers come from fixed size pool,
     *                 unlimitedThresholdBufferSize must be set to a value > 0
     */
    int threshold;
    /*
     * The size of the buffers in the fixed size pool if threshold is UNLIMITED.
     * (common use case, this is set to maxSizeSerialized)
     */
    int unlimitedThresholdBufferSize;
    /* The alignment of the buffers allocated by this pool */
    RTIOsapiAlignment bufferAlignment;
    /* control how the underlying fast buffer pools behave */
    struct REDAFastBufferPoolProperty fastBufferPoolProperty;
} REDAThresholdBufferPoolProperty;


#define REDA_THRESHOLD_BUFFER_POOL_PROPERTY_DEFAULT { \
    0, /* threshold */ \
    0, /* unlimitedThresholdBufferSize */ \
    RTI_OSAPI_ALIGNMENT_DEFAULT, /* bufferAlignment */ \
    REDA_FAST_BUFFER_POOL_PROPERTY_DEFAULT \
}

extern REDADllExport 
struct REDAThresholdBufferPool * REDAThresholdBufferPool_new(
        const struct REDAThresholdBufferPoolProperty *property);

extern REDADllExport 
void REDAThresholdBufferPool_delete(struct REDAThresholdBufferPool *me);

extern REDADllExport 
void * REDAThresholdBufferPool_getBuffer(
        struct REDAThresholdBufferPool *me, 
        int bufferSize);

extern REDADllExport 
void REDAThresholdBufferPool_returnBuffer(
        struct REDAThresholdBufferPool *me,
        void *buffer);
