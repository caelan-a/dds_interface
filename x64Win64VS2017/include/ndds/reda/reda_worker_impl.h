...
/*
 * @(#)reda_worker_impl.h    generated by: makeheader    Fri Apr 23 00:11:20 2021
 *
...
struct REDAWorker {
    ...
    /*i EA with highest level */
    const struct REDAExclusiveArea *_inEaMaxLevelPtr;
    /*i Activity Context of the thread associte to the Worker */
    struct RTIOsapiContext *_activityContext;
};
...
/* ----------------------------------------------------------------- */
#define REDAWorker_getActivityContext(worker) \
    (worker) == NULL \
            ? NULL  \
            : ((struct REDAWorker *) (worker))->_activityContext
/* ----------------------------------------------------------------- */
...
#define REDAWorkerFactory_destroyWorker(__workerFactory, __worker) \
    REDAWorkerFactory_destroyWorkerEx((__workerFactory), (__worker), (__worker))

#define REDA_EXCLUSIVE_AREA_UNNAMED_NAME "UNNAMED_EA"

#define REDAWorkerFactory_createExclusiveArea( \
        __workerFactory, \
        __level) \
    REDAWorkerFactory_createExclusiveAreaEx( \
            (__workerFactory), \
            (__level), \
