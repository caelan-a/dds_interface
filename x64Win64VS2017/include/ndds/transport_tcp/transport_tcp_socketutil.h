...
/*
 * @(#)transport_tcp_socketutil.h    generated by: makeheader    Fri Apr 23 02:30:48 2021
 *
...
struct NDDS_Transport_TCP_IOCPManagerOverlapped
{
    ...
    /* 
     * For NDDS_TRANSPORT_TCP_IOCP_MANAGER_OP_SEND operations, this is the pool
     * from which sendBuffer is obtained. If NULL, the buffer will be obtained
     * from NDDS_Transport_TCP_IOCPManagerProperty::sendBufferPool.
     */
    struct REDAFastBufferPool *sendWriteBufferPool;
};
 ...
 * to perform.
 * 
 * @param sendWriteBufferPool. The fast buffer pool that is used to get the send 
 * buffer for this overlap operation. This parameter
 * only applies to the NDDS_TRANSPORT_TCP_IOCP_MANAGER_OP_SEND operation. 
 * If NULL, the manager will use the 
 * NDDS_Transport_TCP_IOCPManagerProperty::sendBufferPool provided during the
...
/* \ingroup NDDS_TransportTCPSocketGroupComponent
  ...
  @param sendWriteBufferPool. The fast buffer pool that is used to get the send 
  buffer for this overlap operation. Depending on the value of the property
  dds.transport.TCPv4.tcp1.shared_write_buffer_allocation this pool is global
  to the transport instance or specific per destination. This parameter
  only applies to the NDDS_TRANSPORT_TCP_IOCP_MANAGER_OP_SEND operation.
 */
...
struct NDDS_Transport_TCP_SocketGroupEntry;


extern	void * NDDS_Transport_TCP_SocketGroupEntry_getUserData(
        struct NDDS_Transport_TCP_SocketGroupEntry *entry);


struct NDDS_Transport_TCP_SocketGroupEntry *
NDDS_Transport_TCP_SocketGroup_getFirstSocketGroupEntry(
        struct NDDS_Transport_TCP_SocketGroup *self);


struct NDDS_Transport_TCP_SocketGroupEntry *
NDDS_Transport_TCP_SocketGroup_getNextSocketGroupEntry(
        struct NDDS_Transport_TCP_SocketGroup *self,
        struct NDDS_Transport_TCP_SocketGroupEntry *entry);
