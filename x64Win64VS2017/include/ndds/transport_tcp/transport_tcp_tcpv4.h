...
/*
 * @(#)transport_tcp_tcpv4.h    generated by: makeheader    Fri Apr 23 02:30:48 2021
 *
...
#define NDDS_TRANSPORT_TCPV4_WRITE_BUFFER_POOL_GROWTH_POLICY_DEFAULT {4L, 32L, 2L}
...
struct NDDS_Transport_TCPv4_Property_t {
    ...
    */
    ...
    /*e \brief Sets the interface tracker detection kind to polling
     * for detecting changes in the interfaces. Only used when using
     * Transport TCP Plugin LAN mode
     * (class-id NDDS_TRANSPORT_CLASSID_TCPV4_LAN).
     *
     *  See NDDS_Transport_UDPv4_Property_t#force_interface_poll_detection
     *  for more details.
    */
    ...
    */
    ...
    /*e \brief Public locator (IP address and port) of the transport instantiation.
        ...
        \default \ref NDDS_TRANSPORT_ADDRESS_INVALID_INITIALIZER
     */
...
The server_bind_port cannot be shared among multiple participants on a common host. On most operating systems, attempting to reuse the same server_bind_port for multiple participants on a common host will result in a "port already in use" error. However, Windows systems will not recognize if the server_bind_port is already in use, and thus care must be taken to properly configure Windows systems.
    ...
    */
    ...
    /*e \brief Allocation settings applied to buffers used for asynchronous 
        (nonblocking) send. To enable asynchronous send, set the field 
        \ref force_asynchronous_send to 1.

        These settings configure the initial number of buffers, the maximum
        number of buffers and the buffers to be allocated when more buffers are
        needed. 

        The pool of buffers can be configured to be shared across all the TCP 
        connections created by the TCP Transport or to be exclusive for a 
        connection by setting the property \ref shared_write_buffer_allocation 
        to 1. The default value is 0.

        Note that for the write buffer pool, the max_count is not set to 
        unlimited. This is to avoid having a fast writer quickly exhaust all 
        the available system memory, in case of a temporary network slowdown. 
        When this write buffer pool reaches the maximum, a new message will 
        replace the oldest message that is not currently in the process 
        of being sent. This guarantees that new messages are prioritized, 
        while at the same time not running into a situation in which messages 
        are not received. Messages that are replaced and not sent may be resent 
        later depending on the application's QoS (if the transport is used for 
    ...
    */
    ...
    /*e \brief Determines whether the pool of buffers created with asynchronous 
       (non-blocking) send is shared or exclusive per TCP connection.

        Sharing this buffer across connections may lead to less memory 
        consumption. However, high-throughput connections may starve 
        low-throughput connections. This is why the default value is 0.

        The size of the buffer pool can be configured using the field 
        \ref write_buffer_allocation.

        \default 0 (allocation settings per TCP connection)
    */
    RTI_INT32 shared_write_buffer_allocation;

    /*i \brief Indicates if the oldest write buffer, that is not in the process
        of being sent, must be replaced when there are no more write buffer 
        available.

        \default 1
    */
    RTI_INT32 enable_write_buffer_replacement;
    ...
    */
    ...
    /*e \brief Controls whether the plugin will send data synchronously or asynchronously.
        ...
        When this parameter is set to 0, the TCP Transport will attempt to send 
        data as soon as the internal send() function is called. When it is set 
        to 1, the transport will make a copy of the data to send in an internal 
        send buffer and enqueue it. Data will be sent as soon as the 
        low-level socket buffer has space.

        Setting this option to 0 (default) should provide better latency. 
        However, in high-throughput scenarios, a 0 setting may cause the 
        low-level send() function to block until the data is physically 
        delivered to the lower socket buffer. For an application writing data 
        at a very fast rate, the 0 setting may cause the caller thread to 
        block if the send socket buffer is full. This could produce lower 
        throughput in those conditions (the caller thread could prepare the 
        next packet while waiting for the send socket buffer to become 
        available).

        The size of the buffer pool created by setting this option 1 can be 
        configured using the field \ref write_buffer_allocation. In addition, 
        the TCP Transport can be used to create one buffer pool per connection 
        or a single buffer pool shared across all TCP connections by using the 
        field \ref shared_write_buffer_allocation.
    ...
    /*e \brief Bitmap that specifies the verbosity of log messages from the transport.
        ...
        <ul>
            ...
            <li>0x10: remote
            <li>0x20: period
            <li>0x100: other (used for control protocol tracing)
        </ul>
...
#define NDDS_TRANSPORT_TCPV4_PROPERTY_DEFAULT_LAN { \
 ...
 NDDS_TRANSPORT_ADDRESS_INVALID_INITIALIZER, /* public_address */ \
 ...
 0, /* shared_write_buffer_allocation */ \
 1, /* enable_write_buffer_replacement */ \
