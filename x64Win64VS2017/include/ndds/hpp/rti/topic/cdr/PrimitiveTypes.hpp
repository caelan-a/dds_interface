...
#define RTI_TOPIC_CDR_SPECIALIZE_PRIMITIVE_TYPE_TRAITS_DEF(              \
        TYPE,                                                            \
        CDR_ENUM,                                                        \
        CDR_TYPE,                                                        \
        SERIALIZE_MACRO)                                                 \
    ...
    {                                                                    \
        RTICdrStream_##SERIALIZE_MACRO##Fast(stream, (CDR_TYPE *) v);    \
        return true;                                                     \
    }                                                                    \
    static bool deserialize_fast(RTICdrStream *stream, TYPE *v)          \
    {                                                                    \
        RTICdrStream_de##SERIALIZE_MACRO##Fast(stream, (CDR_TYPE *) v);  \
        return true;                                                     \
    }
...
#define RTI_TOPIC_CDR_SPECIALIZE_PRIMITIVE_TYPE_TRAITS(     \
    ...
    struct primitive_type_traits<TYPE> {                    \
        RTI_TOPIC_CDR_SPECIALIZE_PRIMITIVE_TYPE_TRAITS_DEF( \
                TYPE,                                       \
                CDR_ENUM,                                   \
                CDR_TYPE,                                   \
                SERIALIZE_MACRO)                            \
...
// Specializations for int64_t (and uin64_t) need to be defined only when
// int64_t and rti::core::int64 are not exactly the same type
template<typename T>
struct primitive_type_traits<
        T,
        typename rtiboost::enable_if_c<
                rtiboost::is_same<T, int64_t>::value
                && !rtiboost::is_same<T, rti::core::int64>::value>::type> {
    RTI_TOPIC_CDR_SPECIALIZE_PRIMITIVE_TYPE_TRAITS_DEF(
            int64_t,
...
struct primitive_type_traits<
        T,
        typename rtiboost::enable_if_c<
                rtiboost::is_same<T, uint64_t>::value
                && !rtiboost::is_same<T, rti::core::uint64>::value>::type> {
    RTI_TOPIC_CDR_SPECIALIZE_PRIMITIVE_TYPE_TRAITS_DEF(
            uint64_t,
