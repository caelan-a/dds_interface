...
#define RTI_MEM_FN std::mem_fn // since c++11
#else
#define RTI_MEM_FN std::mem_fun // deprecated in c++11, removed in c++17
#endif
    ...
    TestFunc test_function)
{
    ...
    RTITestLog_logWithFunctionName(
            RTI_LOG_BIT_WARN,
            function_name.c_str(),
            &RTI_LOG_ANY_s,
            "?");
    ...
    struct runner_ ## TEST_FUNCTION {                                         \
        ...
        {                                                                     \
            return run_test_function(                                         \
                ...
                RTI_MEM_FN(&CLASS_NAME::TEST_FUNCTION));                      \
        }                                                                     \
    };                                                                        \
                                                                              \
...
// The RTI_TEST_FUNCTION_ADD_BASICPUBSUB macro was created to support running a 
// basic publish subscribe test as part of CORE_BASICPUBSUB_VALIDATION_ARCHITECTURES. 
// Tests added with this macro will belong to _test_functions_basic_pubsub test set
#define RTI_TEST_FUNCTION_ADD_BASICPUBSUB(CLASS_NAME, TEST_FUNCTION)          \
    struct runner_basicpubsub_ ## TEST_FUNCTION {                             \
        static int run(const struct RTITestSetting* setting)                  \
        {                                                                     \
            return run_test_function(                                         \
                setting,                                                      \
                ...
                RTI_MEM_FN(&CLASS_NAME::TEST_FUNCTION));                      \
                                                                              ...
                                                                              \
    add_test_function_basic_pubsub((RTITestFunction) runner_basicpubsub_ ## TEST_FUNCTION::run)
...
public:
    ...
    int get_test_count_basic_pubsub() const
    {
        return static_cast<int>(_test_functions_basic_pubsub.size());
    }
    ...
    const RTITestFunction* get_test_functions_basic_pubsub() const
    {
        return (_test_functions_basic_pubsub.empty() 
                ? NULL 
                : &_test_functions_basic_pubsub[0]);
    }
    ...
    void add_test_function_basic_pubsub(RTITestFunction fnc)
    {
        _test_functions_basic_pubsub.push_back(fnc);
    }
    ...
    {
        int begin, end;
        RTITestFunction *testSelectionBasic = NULL, *testSelectionAdditional = NULL;
        int testSelectionBasicSize = 0, testSelectionAdditionalSize = 0;

        RTITestSetting_getRange(settings, &begin, &end, _test_level);
        if (RTITestSetting_isCoreBasicPubSubValidationArchitecture() 
                && !RTITestSetting_isRangeSpecified(begin, end)) {
            testSelectionBasic = (RTITestFunction *) get_test_functions_basic_pubsub();
            testSelectionBasicSize = get_test_count_basic_pubsub();
            testSelectionAdditional = NULL;
            testSelectionAdditionalSize = 0;
        } else {
            testSelectionBasic = (RTITestFunction *) get_test_functions();
            testSelectionBasicSize = get_test_count();
            testSelectionAdditional = (RTITestFunction *) get_test_functions_additional();
            testSelectionAdditionalSize = get_test_count_additional();
        }

        DDSLog_setBitmaps(
                settings->submoduleMask, 
                settings->instrumentationMask);
        return RTITestSetting_runTestsExt(
                settings, 
                _name.c_str(), 
                testSelectionBasic, 
                testSelectionBasicSize, 
                testSelectionAdditional, 
                testSelectionAdditionalSize, 
                begin, 
                end);
    }
...
private:
    ...
    void merge_tester(Tester& src_tester)
    {
        ...
        const RTITestFunction * src_functions_basic_pubsub 
                = src_tester.get_test_functions_basic_pubsub();
        const int test_count_basicpubsub = src_tester.get_test_count_basic_pubsub();
        ...
        for (int i = 0; i < test_count_basicpubsub; i++) {
            add_test_function_basic_pubsub(src_functions_basic_pubsub[i]);
        }
    }
    ...
    std::vector<RTITestFunction> _test_functions_basic_pubsub;
...
public:
    ...
    template <typename Tester>
    void add_tester_basic_pubsub()
    {
        tester_.add_test_function_basic_pubsub(run_tester<Tester>);
    }
