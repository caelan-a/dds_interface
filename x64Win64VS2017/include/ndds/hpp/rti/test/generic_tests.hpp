...
extern "C" {
// BUILD-2550: Valgrind will report access to uninitialized memory because we
// may be reading bytes used for padding, but we know they are set to 0. (Note
// that this is entirely a test issue.)
// 
// This function avoids name mangling so it can be easily added to the
// suppression file.
inline bool RTICPP2Test_compareMemoryValgrindSuppressed(
    ...
    bool bitwise_comparison = true)
{
    ...
    if (bitwise_comparison) {
        ...
        RTI_TEST_ASSERT(RTICPP2Test_compareMemoryValgrindSuppressed(
                &previous_bytes[0],
                &moved_bytes[0],
                previous_bytes.size()));
    }
