...
public:
     ...
     * - The types accepted by \ref PrimitiveType "PrimitiveType<T>"
     * - `std::string` (or dds::core::string),
     * - DynamicData, when the member type is a nested struct or union. In this
     *   case you can use loan_value() as well.
     * - If the type of the member is an enumeration, use `int32_t`.
     *
     * @note If the member type is a collection type, use loan_value for
     * collections of complex types or set_values for collections of primitive
     * types.
     *
     ...
     *
     * It is not always possible to rely on inferring the type \p T based
     ...
     *
     * @tparam T The element type of the array or sequence. These are the
     * possible types:
     * - The primitive types `char`, `(u)int8_t`, `(u)int16_t`, `DDS_Long`,
     * `DDS_UnsignedLong`, `DDS_LongLong`, `DDS_UnsignedLongLong`, `float`,
     * `double`. The types `int` and `unsigned int` may be used instead of
     * `DDS_Long` and `DDS_UnsignedLong` except in some platforms (VxWorks).
     * - If the element type is an enumeration, use `int32_t`.
     * - If the element type is `bool`, use `uint8_t`.
     *
     * @param name The member name, see \ref
     * DDSDynamicDataModule_member_name_id.
     * @param out_array A vector containing the values of that array or
     * sequence. The vector will be resized accordingly.
     */
    template <typename T>
     ...
     * This function reuses an existing vector
     *
     * @tparam T The element type of the array or sequence. (See
     * \ref get_values(const std::string&, std::vector<T>&) const)
     *
     ...
     *
     * @param index The position of the array or sequence member in the type
     * @param v A vector containing the values to be set for that array or
     * sequence.
     *
     * @see \ref DDSDynamicDataModule_member_name_id
     * @see get_values(const std::string& name, std::vector<T>& out_array) const
    ...
    /**
     * @brief Indicates whether a member exists in this sample
     *
     * If the member doesn't exist in the type, this function returns false. In
     * all other cases, it provides the same result as
     * rti::core::xtypes::DynamicDataMemberInfo::member_exists(), which is
     * retrieved with member_info()).
     *
     * @param name The member name, see \ref
     * DDSDynamicDataModule_member_name_id.
     */
     ...
     * retrieved with member_info()).
     *
     * @param index The member index, see \ref
     * DDSDynamicDataModule_member_name_id.
     */
     ...
     *
     * @pre this->type_kind() must be \idref_TCKind_TK_ARRAY,
     * \idref_TCKind_TK_SEQUENCE, \idref_TCKind_TK_STRUCT, or
     * \idref_TCKind_TK_UNION
     *
     * When `name` identifies a field in a struct or union:
     * - If the specified member is not defined in the type, this function throws
     *   dds::core::InvalidArgumentError.
     * - If the specified member is defined in the type but doesn't exist in
     *   this data sample, this function returns an object with
     *   rti::core::xtypes::DynamicDataMemberInfo::member_exists() set to false.
     * - If the specified member is defined in the type and exists in this data
     *   sample, rti::core::xtypes::DynamicDataMemberInfo::member_exists is
     *   true.
     *
     * When `name` identifies a sequence element, such as `"my_sequence[i]"`:
     * - If `i` is greater than the sequence's maximum length, this
     *   function throws dds::core::InvalidArgumentError.
     * - If `i` is greater than the sequence's current length but smaller
     *   than its maximum length, this function returns an object with
     *   rti::core::xtypes::DynamicDataMemberInfo::member_exists() set to false.
     * - If `i` is smaller than or equal to the current length,
     *   rti::core::xtypes::DynamicDataMemberInfo::member_exists() is
     *   true.
     *
     * When `name` identifies an array element, such as `"my_array[i]`,
     * this function either throws when the `i` is out of bounds or else returns
     * an object with rti::core::xtypes::DynamicDataMemberInfo::member_exists()
     * set to true.
     *
     * @param name The member name, see \ref DDSDynamicDataModule_member_name_id.
     * @return Information about the member.
     * @throw dds::core::InvalidArgumentError if the member doesn't exist in the
     * type, or \ref DDSReturnTypesModule_std_retcodes.
     */
     ...
     *
     * @pre this->type_kind() must be \idref_TCKind_TK_ARRAY,
     * \idref_TCKind_TK_SEQUENCE, or \idref_TCKind_TK_STRUCT,
     * \idref_TCKind_TK_UNION
     *
     * When this sample represents a struct and `index` is the position
     * of the member in the type definition, or when the sample represents a
     * union and `index` is the discriminator value:
     * - If the specified member is not defined in the type, this function
     ...
     *   true.
     *
     * When this sample represents a sequence and `index` is the 1-based element
     * index:
     * - If `index` is greater than the sequence's maximum length, this function
     *   throws dds::core::InvalidArgumentError.
     * - If `index` is greater than the sequence's current length but smaller
     *   than its maximum length, this function returns an object with
     *   rti::core::xtypes::DynamicDataMemberInfo::member_exists() set to false.
     * - If `index` is smaller than or equal to the current length,
     *   rti::core::xtypes::DynamicDataMemberInfo::member_exists() is
     *   true.
     *
     * When this sample represents an array this function either throws
     * dds::core::InvalidArgumentError when the index is out of bounds or else
     * returns an object with
     * rti::core::xtypes::DynamicDataMemberInfo::member_exists() set to true.
     *
     * @param index The member index, see \ref
     * DDSDynamicDataModule_member_name_id.
     * @return DynamicDataMemberInfo Information about the member.
     * @throw dds::core::InvalidArgumentError if the member doesn't exist in the
     * type or the element with this index doesn't exist in this sequence. See
     * also \ref DDSReturnTypesModule_std_retcodes.
     */
