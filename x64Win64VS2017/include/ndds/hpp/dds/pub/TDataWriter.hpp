...
public:
  ...
  DataWriter(
    ...
    * See \ref NDDS_TransportBuiltinsComponent for details on when a builtin
    ...
    * @param the_listener The DataWriter listener. The caller owns the listener
    * and is responsible for deleting it only after resetting it or after
    * deleting the DataWriter.
    ...
    * @see dds::pub::qos::DataWriterQos::operator=(const
    * dds::topic::qos::TopicQos&) which allows assigning the contents of a
    * TopicQos into a DataWriterQos
    * @see listener()
    */
   DataWriter(
           const dds::pub::Publisher& pub,
 ...
 */
  ...
  /**
   * @brief Requests the middleware to delete the instance identified by the instance handle.
   *
   ...
   *
   * @deprecated The use of `set_listener()` is recommended. Unlike this function,
   * `set_listener` receives a `shared_ptr` which simplifies the management
   * of listener's lifecycle.
   *
...
#ifndef RTI_NO_CXX11_SMART_PTR
  /**
   * @brief Sets the listener associated with this writer.
   *
   * The writer will hold a `shared_ptr` to the listener argument,
   * ensuring that it is not deleted at least until this writer is deleted
   * or the listener is reset with `set_listener(nullptr)`.
   *
   * @warning It's recommended that the listener implementation doesn't hold
   * a permanent reference to the writer. If it does, the application needs to
   * manually reset the listener or manually close this writer to ensure
  ...
  /**
   * @brief Returns the listener currently associated with this writer.
   *
