...
public:
    ...
    public:
         ...
         * be selected
         *
         * @see \ref DDSReaderExampleModule_select_samples for an example
         */
         ...
         * the SampleState, ViewState, and InstanceState of a sample. For
         * example:
         *
         * \code
         ...
         * \endcode
         *
         * If you're only interested in the SampleState, the ViewState, or
         * the InstanceState, you can pass it directly. For example, to read alive
         * instances with any SampleState or ViewState:
         *
         * \code
         * LoanedSamples<Foo> samples = reader.select()
...
public:
     ...
     * create the datareader while another thread may be simultaneously
     * calling
     * dds::sub::Subscriber::default_datareader_qos(dds::sub::qos::DataReaderQos).
     *
     * @pre If sub is enabled, the topic must be enabled. If it is not,
     * this operation will fail and no DataReader will be created.
     *
     ...
     * @see dds::sub::qos::DataReaderQos for rules on consistency among QoS
     * @see dds::core::QosProvider::datareader_qos()
     * @see dds::sub::qos::DataReaderQos::operator=(const
     * dds::topic::qos::TopicQos&) which allows assigning the contents of a
     * TopicQos into a DataReaderQos
     */
    DataReader(
            const dds::sub::Subscriber& sub,
            const dds::topic::Topic<T>& topic,
     ...
     * @brief Sets the listener associated with this reader
     *
     * The DataReader will hold a `shared_ptr` to the listener argument,
     * ensuring that it is not deleted at least until this DataReader is deleted
     * or the listener is reset with `set_listener(nullptr)`.
     *
     * @warning It's recommended that the listener implementation doesn't hold
     * a permanent reference to the reader. If it does, the application needs to
     * manually reset the listener or manually close this DataReader to ensure
