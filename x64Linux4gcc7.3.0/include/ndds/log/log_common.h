...
/*
 * @(#)log_common.h    generated by: makeheader    Fri Nov 15 12:21:36 2019
 *
...
/*e \ingroup RTILogBitmapClass 
    ...
    When using this default bitmap, only exceptions
    will be logged.
*/
...
/*e \ingroup RTILogNumberBitmapClass
      ...
      @see RTI_LOG_MODULE_MASK
*/
#define MODULE_LBEDISCOVERY (0x230000)
/*e \ingroup RTILogNumberBitmapClass
...
/*i
  \brief RTILogLevel idenfity the logLevel of a message.
  The logLevel is used for:
        - Filtering what messages are logged according to the configured verbosity.
        - Identifying the logged message by its logLevel.
        - Setting the logged message printFormat.
*/
...
typedef unsigned int RTILogPrintBitmap;
...
*/
#define RTI_LOG_PRINT_FORMAT_DEFAULT \
    (RTI_LOG_PRINT_BIT_MSG | RTI_LOG_PRINT_BIT_AT_METHOD)

/* \ingroup RTILogPrintBitmapClass
   @brief Default print format for the logger in log.1.0
*/
#define RTI_LOG_PRINT_FORMAT_DEBUG \
...
#define RTILog_appendFormatString( \
        ...
        colorValue) \
{ \
    int localAvailableSize = availableSize; \
    /* Make sure the format (potentially, with color), fits the buffer */ \
    int actualAvailableSize = availableSize; \
    \
    if (setColor) { \
        actualAvailableSize -= (int) strlen(colorValue); \
    } \
    if (resetColor) { \
        actualAvailableSize -= (int) strlen(RTI_LOG_COLOR_RESET); \
    } \
    \
    if (strlen(formatString) <= actualAvailableSize) { \
        ...
        if (resetColor) { \
            localAvailableSize = RTI_LOG_CONTEXT_LENGTH_MAX - strlen(destBuffer) - 1; \
...
int RTILog_vdebug(RTILogBitmap instrumentBit, const char *errMsg, va_list ap);

extern RTILogDllExport int RTILog_debug(const char *errMsg, ...);

extern RTILogDllExport int RTILog_debugWithInstrumentBit(
        RTILogBitmap instrumentBit,
        const char *errMsg,
        ...);

extern RTILogDllExport unsigned int RTILog_printString(
        const char *string,
        unsigned int stringLen);

extern RTILogDllExport int RTILog_print(
        RTILogBitmap instrumentBit,
...
extern RTILogDllExport
int RTILog_printLocationContextAndMsg(
        RTILogBitmap instrumentBit,
        unsigned int module,
        const char *filename,
        const char *method,
...
#if defined(RTI_LOG_EXTENDED)
#define RTILog_logWithParams( \
        MODULE, \
        INSTRUMENT_MASK, \
        SUBMODULE_MASK, \
        INSTRUMENT_BIT, \
...
#if defined(RTI_LOG_EXTENDED)
#define RTILog_logWithFunctionName(\
        MODULE, \
        INSTRUMENT_MASK, \
        SUBMODULE_MASK, \
        INSTRUMENT_BIT, \
...
#if defined(RTI_LOG_EXTENDED)
#define RTILog_logWithImplicitFunctionName(\
        MODULE, \
        INSTRUMENT_MASK, \
        SUBMODULE_MASK, \
        INSTRUMENT_BIT, \
...
#define RTILog_logFreeForm( \
        ...
        if (((INSTRUMENT_MASK) & (INSTRUMENT_BIT)) \
                && ((SUBMODULE_MASK) & (CURRENT_SUBMODULE))) \
            RTILog_debugWithInstrumentBit(INSTRUMENT_BIT, __VA_ARGS__)

#define RTILog_logFatalWithContext( \
        INSTRUMENT_MASK, \
        SUBMODULE_MASK, \
        INSTRUMENT_BIT, \
        CURRENT_SUBMODULE, \
        ...
        if (((INSTRUMENT_MASK) & (INSTRUMENT_BIT)) \
                && ((SUBMODULE_MASK) & (CURRENT_SUBMODULE))) \
            RTILog_printContextAndFatalMsg(INSTRUMENT_BIT, __VA_ARGS__)
...
#define RTILog_logWithContext(\
        ...
        if (((INSTRUMENT_MASK) & (INSTRUMENT_BIT)) \
                && ((SUBMODULE_MASK) & (CURRENT_SUBMODULE))) \
            RTILog_printContextAndMsg(INSTRUMENT_BIT, __VA_ARGS__)

#define RTILog_printMessage( \
        INSTRUMENT_BIT, \
        MODULE, \
        ...) \
    RTILog_printLocationContextAndMsg(\
  ...
  #define RTILog_checkPrecondition(MODULE, preconditionExpression, failAction) \
    if (preconditionExpression) { \
        ...
        if (RTILog_dumpBacktrace != NULL) { \
            RTILog_dumpBacktrace(RTI_LOG_BIT_EXCEPTION); \
        } \
        failAction; \
    }
#else /* do nothing */
...
extern RTILogDllVariable RTILogPrintBitmap RTILog_printMask[RTI_LOG_COUNT_LOG_LEVELS];

extern RTILogDllExport RTILogPrintBitmap RTILog_getPrintMask(void);

extern RTILogDllExport void RTILog_setPrintMask(RTILogPrintBitmap mask);

extern RTILogDllExport void RTILog_appendPrintMask(RTILogPrintBitmap mask);

extern RTILogDllExport void RTILog_setPrintMaskByLogLevel(
        RTILogPrintBitmap mask,
        RTILogLevel logLevel);

extern RTILogDllExport RTILogPrintBitmap RTILog_getPrintMaskByLogLevel(RTILogLevel logLevel);

typedef void (*RTILogBacktraceMethod)(RTILogBitmap logLevel);

extern RTILogDllVariable RTILogBacktraceMethod RTILog_dumpBacktrace;


/*
 * This functionality is already defined in osapi.1.0 module:
 *      RTIOsapiUtility_unusedParameter()
 * RTILog_unusedParameter() will only be used in log.1.0.
 */
#define RTILog_unusedParameter(parameter) (void) parameter
